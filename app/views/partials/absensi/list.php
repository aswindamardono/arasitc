    ...
    if ($show_header == true) {
        ...
        <div class="bg-light p-3 mb-3">
            <div class="container-fluid">
                <div class="row ">
                    <div class="col ">
                        <h4 class="record-title">Absensi</h4>
                    </div>
                    <div class="col-sm-3 ">
                        <?php if ($can_add) { ?>
                                    ...
                                    <ul class="breadcrumb m-0 p-1">
                                        ...
                                        if (!empty($field_name)) {
                                            ...
                                            <li class="breadcrumb-item">
                                                <a class="text-decoration-none" href="<?php print_link('absensi'); ?>">
            ...
            <div class="row ">
                <div class="col-md-12 comp-grid">
                    ...
                    <div class="filter-tags mb-2">
                        <?php
                        if (!empty($_GET['absensi_jam_msk'])) {
                            ?>
                            <div class="filter-chip card bg-light">
                                <b>Absensi Jam Msk :</b>
                                <?php
                                $date_val = get_value('absensi_jam_msk');
                                $formated_date = "";
                                ...
                                echo $formated_date;
                                $remove_link = unset_get_value('absensi_jam_msk', $this->route->page_url);
                                ?>
                    ...
                    <div class=" animated zoomInDown page-content">
                                ...
                                <table class="table  table-striped table-sm text-left">
                                    <thead class="table-header bg-light">
                                        <tr>
                                            ...
                                            <?php } ?>
                                            <th class="td-sno">#</th>
                                            <th class="td-username_absensi"> Username Absensi</th>
                                            <th class="td-jam_msk"> Jam Masuk</th>
                                            <th class="td-jam_klr"> Jam Keluar</th>
                                            <th class="td-btn"></th>
                                        </tr>
                                    </thead>
                                    <?php
                                    if (!empty($records)) {
                                        ...
                                        <tbody class="page-data" id="page-data-<?php echo $page_element_id; ?>">
                                            ...
                                            foreach ($records as $data) {
                                                $rec_id = (!empty($data['id_absen']) ? urlencode($data['id_absen']) : null);
                                                $counter++;
                                                ?>
                                                <tr>
                                                    <?php if ($can_delete) { ?>
                                                        <th class=" td-checkbox">
                                                            <label class="custom-control custom-checkbox custom-control-inline">
                                                                <input class="optioncheck custom-control-input" name="optioncheck[]"
                                                                    value="<?php echo $data['id_absen'] ?>" type="checkbox" />
                                                                <span class="custom-control-label"></span>
                                                            </label>
                                                        </th>
                                                    <?php } ?>
                                                    <th class="td-sno"><?php echo $counter; ?></th>
                                                    <td class="td-username_absensi">
                                                        <span <?php if ($can_edit) { ?>
                                                                data-value="<?php echo $data['username_absensi']; ?>"
                                                                data-pk="<?php echo $data['id_absen'] ?>"
                                                                data-url="<?php print_link("absensi/editfield/" . urlencode($data['id_absen'])); ?>"
                                                                data-name="username_absensi" data-title="Enter Username Absensi"
                                                    ...
                                                    <td class="td-jam_msk">
                                                        <?php echo Html::format_datetime_badge($data['jam_msk'], 'masuk'); ?>
                                                    </td>
                                                    <td class="td-jam_klr">
                                                        <?php
                                                        ...
                                                        ?>
                                                        <span <?php if ($can_edit) { ?>
                                                                data-value="<?php echo $data['jam_klr']; ?>"
                                                                data-pk="<?php echo $data['id_absen'] ?>"
                                                                data-url="<?php print_link("absensi/editfield/" . urlencode($data['id_absen'])); ?>"
                                                                data-name="jam_klr" data-title="Enter Jam Klr" data-placement="left"
                                                        ...
                                                        <?php if ($jam_klr_empty && $can_edit) { ?>
                                                            <a class="btn btn-sm has-tooltip"
                                                                href=" <?php print_link("absensi/edit/$rec_id"); ?>"
                                                                style="background: linear-gradient(135deg, #ff6b6b, #ee5a24); color: #fff; border: none; border-radius: 50%; width: 34px; height: 34px; display: inline-flex; align-items: center; justify-content: center; box-shadow: 0 3px 10px rgba(238,90,36,0.35); transition: all 0.2s ease; margin-left: 6px; vertical-align: middle;">
                                                    ...
                                                    <th class="td-btn">
                                                        <?php if ($can_view) { ?>
                                                            <a class="btn btn-sm btn-success has-tooltip" title="View Record"
                                                                href="<?php print_link("absensi/view/$rec_id"); ?>">
                                                                <i class="fa fa-eye"></i>
                                                            </a>
                                                        <?php } ?>
                                                        <?php if ($can_delete) { ?>
                                                            <a class="btn btn-sm btn-danger has-tooltip record-delete-btn"
                                                                ...
                                                                href="<?php print_link("absensi/delete/$rec_id/?csrf_token=$csrf_token&redirect=$current_page"); ?>"
                                            ...
                                            <div class="p-3 d-flex justify-content-between">
                                                <?php if ($can_delete) { ?>
                                                    <button data-prompt-msg="Are you sure you want to delete these records?"
                                                        data-display-style="modal"
                                                        data-url="<?php print_link("absensi/delete/{sel_ids}/?csrf_token=$csrf_token&redirect=$current_page"); ?>"
            ...
            <div class="row ">
                <div class="col-md-12 comp-grid">
                    <form method="get" action="<?php print_link($current_page) ?>" class="form filter-form">
                        <div class="card mb-3">
                            <div class="card-header h4 h4">Filter Absensi</div>
                            <div class="p-2">
                                <input class="form-control datepicker"
                                    value="<?php echo $this->set_field_value('absensi_jam_msk') ?>" type="datetime"
                                    name="absensi_jam_msk" placeholder="Klik disini untuk mencari tanggal"
